...
int getPageProtection(void *addr) {
    ...
    while (fgets(line, sizeof(line), f)) {
        uintptr_t start, end;
        char prot[8];
        if (sscanf(line, "%" PRIxPTR "-%" PRIxPTR " %s %*s %*s %*s %*s", &start, &end, prot)) {
...
void setPageProtection(void *addr, int prot) {
    void *page = (void *)((uintptr_t)addr & ~(PAGE_SIZE - 1));
    size_t len = ((uintptr_t)addr + PAGE_SIZE) - (uintptr_t)page;
    mprotect(page, len, prot);
}

uint32_t CalculateChecksum(const unsigned char* buffer, size_t length) {
    uint32_t checksum = 0;
...
void Tools::Hook(void * const target, void * const replace, void ** const backup) {
    int prot = getPageProtection(target);
    setPageProtection(target, PROT_READ | PROT_WRITE | PROT_EXEC);
    DobbyHook(target, replace, backup);
    setPageProtection(target, prot);
}
