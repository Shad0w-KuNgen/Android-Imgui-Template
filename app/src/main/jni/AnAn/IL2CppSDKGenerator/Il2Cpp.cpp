...
#include "Il2Cpp.h"
// ========================================================================================================================================== //
#define IL2CPP__TAG "AnAn—Il2CppSdk"
...
void *Il2Cpp::GetClass(const char *image, const char *namespaze, const char *clazz) {
    ...
    if(classes.size() > 1)
    {
        ...
        IL2CPP_LOGE("Can't find subclass %s in class %s!", classes[1].c_str(), classes[0].c_str());
        return 0;
    }
            ...
            for (int i = 0; i < argsCount; i++) {
                ...
                if (arg) {
                    ...
                    if (strcmp(tname, args[i]) == 0) {
                        ...
                        IL2CPP_LOGE("Argument at index %d didn't matched requested argument!\r\n\tRequested: %s\r\n\tActual: %s\r\nSkipping function...", i, args[i], tname);
                        score = 0;
goto skip;
                    }
                }
        ...
        if(score == argsCount)
        {
            IL2CPP_LOGE("Found matched function!");
...
vector<const char*> Il2Cpp::getNameMethod(const char *image, const char *namespaze, const char *clazz)
{
    vector<const char*> tmpList;
    void *img = Il2Cpp::GetImage(image); 
    if(!img) {
        return tmpList;
    }
    void *klass = Il2Cpp::GetClass(image, namespaze, clazz);
    if(!klass) {
        return tmpList;
    }
    void *iter = 0;
...
vector<void*> Il2Cpp::GetOffset(const char *image, const char *namespaze, const char *clazz) {
    vector<void*> tmpList;
    void *img = Il2Cpp::GetImage(image); 
    if(!img) {
        return tmpList;
