...
Define a class and inherit zygisk::ModuleBase to implement the functionality of
your module. Use the macro REGISTER_ZYGISK_MODULE(className) to register that
class to Zygisk.

Please note that modules will only be loaded after zygote has forked the child
process. THIS MEANS ALL OF YOUR CODE RUNS IN THE APP/SYSTEM SERVER PROCESS, NOT
THE ZYGOTE DAEMON!

Example code:

static jint (*orig_logger_entry_max)(JNIEnv *env);
static jint my_logger_entry_max(JNIEnv *env) { return
orig_logger_entry_max(env); }
...
public:
    ...
    void preAppSpecialize(zygisk::AppSpecializeArgs *args) override {
        JNINativeMethod methods[] = {
            { "logger_entry_max_payload_native", "()I", (void*)
my_logger_entry_max },
        };
...
enum Option : int {
    ...
    //
    // Set this option to force all Magisk and modules' files to be unmounted from
    // the
    // mount namespace of the process, regardless of the denylist enforcement
    // status.
...
// All API functions will stop working after post[XXX]Specialize as Zygisk will
...
struct Api {
    ...
    //
    // The root companion process is ABI aware; that is, when calling this
    // function from a 32-bit process, you will be connected to a 32-bit companion
    // process, and vice versa for 64-bit.
    //
    ...
    // Accessing the directory returned is only possible in the pre[XXX]Specialize
    // functions or in the root companion process (assuming that you sent the fd
    // over the socket). Both restrictions are due to SELinux and UID.
    //
...
// request from your module running in a target process. The function has to
// accept an integer value, which is a socket that is connected to the target
// process. See Api::connectCompanion() for more info.
//
